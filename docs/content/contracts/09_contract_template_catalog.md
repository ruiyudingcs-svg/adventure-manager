...
十二份模板已经按 `docs/content/dragon_invasion/17_dragon_invasion_scenario_spec.md` 的阵营目标与进度钟，以及 `docs/content/dragon_invasion/18_dragon_invasion_problem_and_action_catalog.md` 的问题和行动完成覆盖审计。模板总数固定为十二份。

本文件不重复保存合同的完整实现数值。三份基准合同见
`docs/rules/contracts/10_baseline_contracts.md`；其余九份合同的精确 check 难度、五档数值、疲劳、
伤病率、条款和 MissionContextDelta 已由 Gate E 固定在
`docs/rules/contracts/21_remaining_contract_numerical_rules.md`。合同来源、资格和筛选规则由
`docs/rules/campaign/11_contract_proposal_generation.md` 固定；精确世界状态实例绑定见
`docs/rules/campaign/12_contract_instantiation_and_commitment.md`。
...
带 NPC 路径的七项行动已经在 `docs/content/dragon_invasion/18_dragon_invasion_problem_and_action_catalog.md` 的 `weekly_action_table` 中定义，并与合同共享 target lock。所有恶化效果来自对应问题定义。Agenda 来源 Offer 运行时没有问题锚点，因此 NPC 路径不可执行后的恶化分支降为自然过期。
...
`docs/rules/contracts/10_baseline_contracts.md`；其余九份合同必须按
`docs/rules/contracts/21_remaining_contract_numerical_rules.md` 拆分到逐阶段五档结果，本文的净值
