...
Gate B 的 trait 手段规则只允许 `docs/rules/contracts/15_staged_contract_resolution_rules.md`
...
`docs/rules/contracts/15_staged_contract_resolution_rules.md` 第 5.1 节；该向量和禁忌强度都是
...
V0.1 只支持 `all_conditions`。Bonus 条款不得配置失败效果或结果封顶。条款不直接修改世界钟、成员士气、成员关系、消息或后续合同。完整规则见 `docs/rules/contracts/08_contract_clause_spec.md`。
...
规则按稳定 ID 求值，全部匹配数值先求和后裁剪。单条 check 难度修正为 -10 至 +10，同一 check 汇总到 -20 至 +20；单条初始上下文修正为 -3 至 +3，同一 key 汇总到 -6 至 +6，再应用到默认值并裁剪到 0 至 10。完整规则见 `docs/rules/campaign/12_contract_instantiation_and_commitment.md`。
...
`docs/rules/contracts/15_staged_contract_resolution_rules.md` 第 5.2—5.5 节。
...
`relationship_values` 是 `starting_relationships + relationship_deltas` 的独立稀疏有向副本，key 为目标成员 ID。进入合同结算的有效值必须在 -100 至 +100；缺少 key 表示没有显式记录，而不是显式的 0。Resolver 按 `docs/rules/contracts/15_staged_contract_resolution_rules.md` 第 4.1 节从四人快照派生六个成员对，不读取 Definition、State 或完整 CampaignState。
...
`docs/rules/campaign/19_content_catalog_state_and_situation_rules.md` 第 5 节。
...
十个整数指标默认都为 0，合法范围都为 0 至 10。一次 check 内同一 key 的全部 delta 先求和，再裁剪一次；不同 check 按阶段与稳定 check ID 顺序更新。`outcome_tags` 和 `used_method_tags` 稳定去重。完整方向、UI 与更新规则见 `docs/rules/contracts/15_staged_contract_resolution_rules.md`。
...
`score` 是 64 位浮点 raw score 按 `round_away` 舍入后得到的不裁剪整数。发生舍入时，`reason_entries` 包含仅 debug 可见的 `score_rounding`，使全部评分原因 amount 之和等于最终 score。完整精度、随机和惩罚规则见 `docs/rules/contracts/15_staged_contract_resolution_rules.md` 第 4 节。
...
`docs/content/contracts/09_contract_template_catalog.md`。三份数值化基准合同见
`docs/rules/contracts/10_baseline_contracts.md`，其余九份完整数值见
`docs/rules/contracts/21_remaining_contract_numerical_rules.md`。每类至少有一份会改变后续合同池。
