...
状态：Milestone 0—4 已完成；Milestone 5 功能实现完成、发布验收中

最后更新：2026-07-26

当前实现总结与交接入口见 `docs/product/23_v0.1_implementation_summary.md`。
V0.1 后的可控性、合同组合、招募和英雄定位路线见
`docs/product/24_post_v0.1_agency_roadmap.md`。
...
五条钟见 `docs/content/dragon_invasion/17_dragon_invasion_scenario_spec.md`；九个问题、十四项行动、合同
覆盖与结局路径见 `docs/content/dragon_invasion/18_dragon_invasion_problem_and_action_catalog.md`。第四批
...
功能实现与自动验证已经完成：planning-only 存档、结局页、引导、错误/空状态、
只读平衡审计和 Windows Desktop preset 均已接入，完整 headless suite 最近一次结果为
233 passed、0 failed。Milestone 5 尚未关闭，因为当前开发机缺少 Godot 4.7 stable
Windows export templates，且三尺寸键盘操作、完整新玩家整局、导出构建启动及至少两周
流程仍需人工验收。
